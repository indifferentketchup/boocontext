 ...
 */
export function extractCircleCIWorkflows(parsed, relPath, rawContent) {
    ...
    for (const [name, wf] of Object.entries(workflows)) {
        if (name === "version")
            continue; // CircleCI sometimes puts version in workflows
        if (!wf || typeof wf !== "object")
            continue;
        const wfObj = wf;
        const jobRefs = Array.isArray(wfObj.jobs) ? wfObj.jobs : [];
        ...
        if (orbs.length > 0)
            pipeline.envVars = orbs.map(o => `orb:${o}`);
        pipelines.push(pipeline);
    }
    return pipelines;
}
function extractJobs(jobRefs, jobDefs) {
    return jobRefs.map(ref => {
        ...
        if (jobConfig?.context) {
            ...
            if (contexts.length > 0)
                result.environment = contexts.join(", ");
        }
...
function extractTriggers(jobRefs, parameters, workflow) {
    const triggers = [];
    ...
    for (const ref of jobRefs) {
        ...
        if (filters.branches) {
            if (!seenEvents.has("push")) {
                const trigger = { event: "push" };
                if (filters.branches.only)
                    trigger.branches = asArray(filters.branches.only);
        ...
        if (filters.tags) {
            if (!seenEvents.has("tag")) {
                const trigger = { event: "tag" };
                if (filters.tags.only)
                    trigger.tags = asArray(filters.tags.only);
                triggers.push(trigger);
                seenEvents.add("tag");
            }
        }
...
function collectEnvironments(jobRefs) {
    const envs = new Set();
    for (const ref of jobRefs) {
        const [, jobConfig] = parseJobRef(ref);
        if (jobConfig?.context) {
