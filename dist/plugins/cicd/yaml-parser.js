...
function consumeBlockScalar(lines, startIdx, parentIndent) {
    const collected = [];
    let i = startIdx;
    let scalarIndent = -1;
    while (i < lines.length) {
...
export function parseFlowSequence(s) {
    // Find matching ]
    let depth = 0;
    let end = -1;
    let inQuote = null;
...
function skipEmpty(lines, startIdx) {
    let i = startIdx;
    while (i < lines.length) {
