 ...
 */
export function extractServiceInfrastructure(matchedBlocks, allBlocks, config) {
 ...
 */
export async function extractEnvironments(tfvarsFiles, serviceName) {
    const environments = {};
...
function extractComponents(blocks) {
    ...
    for (const block of blocks) {
        if (block.blockType !== "module" && block.blockType !== "resource")
            continue;
        // Skip variable/output blocks — they're metadata not components
        if (block.blockType === "resource" && !isComputeResource(block.resourceType))
            continue;
        // Skip non-compute modules (alarms, s3, logging, etc.)
        if (block.blockType === "module" && !isComputeModule(block))
            continue;
...
function isComputeResource(resourceType) {
    return [
        "aws_ecs_service",
...
function isComputeModule(block) {
    const source = block.attributes["source"] ?? "";
    // Positive: compute modules
    if (source.includes("ecs-service") || source.includes("ecs-worker") || source.includes("lambda"))
        return true;
    ...
    if (source.includes("alarm") || source.includes("logging") || source.includes("s3") ||
        ...
        source.includes("iam") || source.includes("deploy"))
        return false;
    // Heuristic: modules with image attribute are likely compute
    if (block.attributes["image"] || block.attributes["container_image"])
        return true;
    // Default: include if it has cpu/memory (likely compute)
    if (block.attributes["cpu"] || block.attributes["memory"])
        return true;
    return false;
}
function inferDeploymentType(moduleSource, resourceType) {
    if (moduleSource) {
        if (moduleSource.includes("ecs-service"))
            return "ecs-fargate";
        if (moduleSource.includes("ecs-worker"))
            return "ecs-worker";
        if (moduleSource.includes("ecs-service-internal"))
            return "ecs-internal";
        if (moduleSource.includes("lambda"))
            return "lambda";
        if (moduleSource.includes("ec2") || moduleSource.includes("instance"))
            return "ec2";
    }
    if (resourceType.includes("lambda"))
        return "lambda";
    if (resourceType.includes("ecs"))
        return "ecs-fargate";
    if (resourceType.includes("instance"))
        return "ec2";
    return "unknown";
}
function detectPublicFacing(block) {
    const attrs = block.attributes;
    if (attrs["alb_listener_arn"] || attrs["https_listener_arn"])
        return true;
    if (attrs["host_headers"])
        return true;
    // Check if any attribute references an ALB
    for (const val of Object.values(attrs)) {
        if (typeof val === "string" && val.includes("module.alb"))
            return true;
    }
    // Internal service modules are not public-facing
    const source = attrs["source"] ?? "";
    if (source.includes("ecs-worker") || source.includes("ecs-service-internal"))
        return false;
    return false;
}
function extractHealthCheck(block) {
    if (block.attributes["health_check_path"])
        return block.attributes["health_check_path"];
    const healthCheck = block.nestedBlocks["health_check"];
    if (healthCheck?.[0]?.attributes["path"])
        return healthCheck[0].attributes["path"];
    return undefined;
}
function extractEnableFlag(block) {
...
function extractEnvVars(blocks) {
    const envVars = [];
    const seen = new Set();
    for (const block of blocks) {
...
function classifyValueSource(value) {
    if (value.startsWith("var."))
        return "variable";
    if (value.includes("module.") || value.includes("aws_") || value.includes("data."))
        return "reference";
    if (value.includes("${")) {
        // Interpolated string — check if it references vars or resources
        if (value.includes("${var."))
            return "variable";
        if (value.includes("${module.") || value.includes("${data.") || value.includes("${aws_"))
            return "reference";
    }
    return "literal";
}
// ─── Secrets ───
function extractSecrets(blocks) {
    const secrets = [];
    const seen = new Set();
    for (const block of blocks) {
...
// ─── Dependencies ───
function extractDependencies(matchedBlocks, allBlocks) {
    const deps = new Set();
...
function describeModuleDep(moduleName, allBlocks) {
    // Try to find the module definition to get its source for a friendlier name
    const moduleBlock = allBlocks.find((b) => b.blockType === "module" && b.label === moduleName);
    if (moduleBlock?.attributes["source"]) {
        const source = moduleBlock.attributes["source"];
        if (source.includes("rds") || source.includes("database"))
            return `RDS (module.${moduleName})`;
        if (source.includes("redis") || source.includes("elasticache"))
            return `Redis (module.${moduleName})`;
        if (source.includes("s3"))
            return `S3 (module.${moduleName})`;
        if (source.includes("sqs"))
            return `SQS (module.${moduleName})`;
        if (source.includes("sns"))
            return `SNS (module.${moduleName})`;
        if (source.includes("vpc"))
            return `VPC (module.${moduleName})`;
        if (source.includes("alb"))
            return `ALB (module.${moduleName})`;
        if (source.includes("efs"))
            return `EFS (module.${moduleName})`;
        if (source.includes("kms"))
            return `KMS (module.${moduleName})`;
        if (source.includes("dynamodb"))
            return `DynamoDB (module.${moduleName})`;
    }
    return `module.${moduleName}`;
}
// ─── IAM Permissions ───
function extractIamPermissions(blocks) {
    const permissions = [];
    for (const block of blocks) {
...
// ─── Observability ───
function extractObservability(matchedBlocks, allBlocks, serviceName) {
    let logGroup;
    const alarms = [];
