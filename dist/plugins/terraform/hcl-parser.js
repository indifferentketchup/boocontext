 ...
 */
export function parseHclFile(content, filePath) {
 ...
 */
export function parseTfvars(content) {
    const result = {};
    const cleaned = stripComments(content);
...
// ─── Block Extraction ───
function parseTopLevelBlocks(content, filePath) {
 ...
 */
export function extractBraceBlock(content, startAfterOpenBrace) {
    let depth = 1;
    ...
    while (i < lines.length) {
        ...
        if (nestedBlockMatch) {
            ...
            if (blockBody !== null) {
                ...
                if (!nestedBlocks[blockName])
                    nestedBlocks[blockName] = [];
 ...
 */
function parseListOfMaps(listBody) {
    const entries = [];
    let i = 0;
...
function extractBracketBlock(content, startAfterOpenBracket) {
    let depth = 1;
