...
import { collectFiles, detectProject, readBoocontextIgnore } from "./scanner.js";
...
export const BRAND = "boocontext";
export async function scan(root, outputDirName, maxDepth, userConfig = {}, quiet = false) {
    ...
    // Step 2: Collect files — merge .boocontextignore + config ignorePatterns
    if (!quiet)
    ...
    const [rawHttpRoutes, schemas, components, libs, configResult, middleware, graph, graphqlRoutes, grpcRoutes, wsRoutes, events, openapi] = await Promise.all([
    ...
    if (openapi.routes.length > 0) {
        ...
        for (const m of openapi.schemas) {
            if (!existingModelNames.has(m.name.toLowerCase()))
                schemas.push(m);
        }
    }
        ...
        for (const plugin of userConfig.plugins) {
            if (plugin.detector) {
                try {
                    ...
                    if (pluginResult.customSections)
                        customSections.push(...pluginResult.customSections);
                }
                catch (err) {
                    if (!quiet)
    ...
    if (!quiet) {
        ...
        if (gqlCount > 0)
            specialCounts.push(`${gqlCount} graphql`);
        if (grpcCount > 0)
            specialCounts.push(`${grpcCount} rpc`);
        if (wsCount > 0)
            specialCounts.push(`${wsCount} ws`);
        if (events.length > 0)
            specialCounts.push(`${events.length} events`);
