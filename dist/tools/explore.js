...
function overlap(haystack, qterms) {
...
function rankFacets(scan, qterms) {
    const cites = [];
    for (const r of scan.routes) {
        const score = overlap(`${r.method} ${r.path} ${r.tags.join(" ")} ${r.file}`, qterms);
        if (score > 0)
            cites.push({ cite: r.file, reason: `${r.method} ${r.path}`, facet: "route", score: score * 3 });
    }
    for (const m of scan.schemas) {
        ...
        if (score > 0)
            cites.push({ cite: m.name, reason: `${m.orm} model ${m.name}`, facet: "schema", score: score * 2 });
    }
    for (const c of scan.components) {
        const score = overlap(`${c.name} ${c.props.join(" ")} ${c.file}`, qterms);
        if (score > 0)
            cites.push({ cite: c.file, reason: `component ${c.name}`, facet: "component", score: score * 2 });
    }
    for (const lib of scan.libs) {
        for (const ex of lib.exports) {
            const score = overlap(`${ex.name} ${ex.signature ?? ""} ${lib.file}`, qterms);
            if (score > 0)
                cites.push({ cite: lib.file, reason: `${ex.kind} ${ex.name}`, facet: "lib", score: score * 2 });
        }
    }
    for (const mw of scan.middleware) {
        const score = overlap(`${mw.name} ${mw.type} ${mw.file}`, qterms);
        if (score > 0)
            cites.push({ cite: mw.file, reason: `${mw.type} middleware ${mw.name}`, facet: "middleware", score });
    }
    for (const ev of scan.events ?? []) {
        const score = overlap(`${ev.name} ${ev.type} ${ev.system} ${ev.file}`, qterms);
        if (score > 0)
            cites.push({ cite: ev.file, reason: `${ev.system} ${ev.type} ${ev.name}`, facet: "event", score });
    }
