...
function classifySeverity(grade, dimensions) {
...
function computeHotspotScore(healthScore, commits, loc, daysSinceLastCommit) {
    if (commits === 0)
        return 0;
...
function runGitLog(root) {
    try {
        const out = execSync('git log -n 100 --numstat --pretty=format:"%H %at"', { cwd: root, timeout: 5000, encoding: "utf8", shell: "/bin/bash" });
...
function runFileLOC(root) {
    try {
        const out = execSync("git ls-files", { cwd: root, timeout: 5000, encoding: "utf8" });
...
export function createSeverityTool(manager) {
    return {
        name: "boocontext_severity",
        description: "Code health with SonarQube-style severity ladder (INFO/MINOR/MAJOR/CRITICAL), software-quality domains (MAINTAINABILITY/RELIABILITY), and CodeScene-style git-aware hotspot prioritization. Returns findings sorted by organizational impact (health x commit frequency).",
        inputSchema: {
            type: "object",
            properties: {
        ...
        async handler(args) {
            ...
            try {
                ...
                const fileLOC = gitUnavailable ? new Map() : runFileLOC(root);
                const findings = (parsed.files ?? []).map((f) => {
