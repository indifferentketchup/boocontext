...
export async function detectRoutes(files, project, config) {
    const routes = [];
...
// --- Ktor (Kotlin) ---
async function detectKtorRoutes(files, project) {
    ...
    for (const file of ktFiles) {
        ...
        const prefixes = new Map(); // offset → prefix
...
function extractKtorParams(path) {
    const params = [];
    const regex = /\{(\w+)\}/g;
    let m;
...
async function detectPHPRoutes(files, project) {
...
// ─── Laravel ─────────────────────────────────────────────────────────────────
async function detectLaravelRoutes(files, project) {
    // Laravel routes live in routes/api.php and routes/web.php
    const routeFiles = files.filter((f) => f.endsWith(".php") &&
...
// ─── ASP.NET Core ─────────────────────────────────────────────────────────────
async function detectAspNetRoutes(files, project) {
...
// ─── Flutter (go_router) ─────────────────────────────────────────────────────
async function detectFlutterGoRoutes(files, project) {
    ...
    for (const file of dartFiles) {
        ...
        if (!content)
            continue;
        if (!content.includes("GoRoute") && !content.includes("go_router"))
            continue;
        const rel = relative(project.root, file).replace(/\\/g, "/");
...
// ─── Vapor (Swift) ────────────────────────────────────────────────────────────
async function detectVaporRoutes(files, project) {
...
// ─── Android ──────────────────────────────────────────────────────────────────
async function detectAndroidRoutes(files, project) {
...
// --- Angular ---
async function detectAngularRoutes(files, project) {
    ...
    for (const file of tsFiles) {
        ...
        const isRoutingFile = /[/\\].*(?:routing|\.routes)\./i.test(file) ||
...
// names are configurable via `BoocontextConfig.rokuScreenHelpers` with a
...
async function detectRokuScreens(files, project, config) {
    const { extractSceneGraphComponent, extractMainSceneScreens, isSceneGraphXml } = await import("../ast/extract-scenegraph.js");
    const { extractBrightScriptNavigationCalls, extractFindNodeBindings } = await import("../ast/extract-brightscript.js");
    ...
    const routes = [];
    const seen = new Map(); // key: path — lets us upgrade VIEW -> MODAL later
    for (const scene of sceneXmls) {
        ...
        if (pairedBrs) {
            ...
            if (pc)
                brsSources.push({ file: pairedBrs, content: pc });
        }
...
async function resolveRoutePrefixes(routes, files, project) {
    ...
    function resolveFullPrefix(file, visited) {
        if (resolvedCache.has(file))
            return resolvedCache.get(file);
        if (visited.has(file))
...
function parseJSMountsToGraph(content, entryDir, files, project, mountEdges, sourceFile) {
...
function resolveJSImport(importPath, entryDir, files, project) {
    ...
    for (const ext of [".ts", ".tsx", ".js", ".mjs", ".jsx"]) {
        ...
        if (hit)
            return relative(project.root, hit).replace(/\\/g, "/");
    }
    // Try /index.ts etc
...
function parsePythonMountsToGraph(content, _entryDir, files, project, mountEdges, sourceFile) {
    // Step 1: Build alias map: "auth_router" → "backend/routes/auth.py"
    //   from routes.auth import router as auth_router
    const aliasRe = /from\s+([\w.]+)\s+import\s+router\s+as\s+(\w+)/g;
    const aliasMap = new Map(); // alias → source file
    let m;
    while ((m = aliasRe.exec(content)) !== null) {
        ...
        if (hit)
            aliasMap.set(alias, relative(project.root, hit).replace(/\\/g, "/"));
    }
