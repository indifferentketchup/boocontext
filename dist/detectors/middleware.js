...
export async function detectMiddleware(files, project) {
    ...
    if (project.frameworks.includes("roku-scenegraph")) {
        const { extractBrightScriptObservers, extractBrightScriptGlobalFields } = await import("../ast/extract-brightscript.js");
        ...
        for (const file of brsFiles) {
            ...
            for (const obs of extractBrightScriptObservers(content)) {
                ...
                if (middleware.some((m) => m.name === name && m.file === rel))
                    continue;
                middleware.push({
                    name,
                    file: rel,
