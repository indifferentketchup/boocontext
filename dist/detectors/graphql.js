...
// ---------- GraphQL ----------
export async function detectGraphQLRoutes(files, project) {
    const routes = [];
    ...
    const tsJsFiles = files.filter((f) => /\.(ts|tsx|js|jsx|mjs)$/.test(f) && !f.includes("node_modules"));
    ...
    for (const file of pyFiles) {
        ...
        if (!content)
            continue;
        if (!content.includes("strawberry") && !content.includes("graphene"))
            continue;
        const rel = relative(project.root, file).replace(/\\/g, "/");
...
function extractSDLOperations(sdl, file) {
    const routes = [];
    ...
    for (const { type, body } of typeBlocks) {
        const method = type.toLowerCase() === "query"
            ? "QUERY"
...
function isInsideMutationClass(content, idx) {
...
// ---------- gRPC ----------
export async function detectGRPCRoutes(files, project) {
    const routes = [];
...
// ---------- WebSocket ----------
export async function detectWebSocketRoutes(files, project) {
    const routes = [];
    const relevantFiles = files.filter((f) => /\.(ts|tsx|js|jsx|mjs|py|rb)$/.test(f) && !f.includes("node_modules"));
    for (const file of relevantFiles) {
        ...
        while ((m = socketOnPat.exec(content)) !== null) {
            ...
            if (eventName === "connection" || eventName === "disconnect")
                continue; // lifecycle, not app events
            routes.push({
                method: "WS",
                path: eventName,
                file: rel,
