 ...
 */
export function extractSceneGraphComponent(content) {
    ...
    if (interfaceBlock) {
        ...
        while ((m = functionPattern.exec(body)) !== null) {
            ...
            if (fnName)
                interfaceFunctions.push(fnName);
        }
    }
 ...
 *   </component>
 */
export function extractMainSceneScreens(content) {
    const result = {};
    const childrenBlock = content.match(/<children\b[^>]*>([\s\S]*?)<\/children>/i);
    if (!childrenBlock)
        return result;
 ...
 */
export function isSceneGraphXml(content) {
    if (!/<component\b/i.test(content))
        return false;
    if (!/\bname\s*=/i.test(content))
        return false;
    ...
    if (/xmlns:android\s*=/.test(content))
        return false;
    return true;
}
function normalizeSceneGraphType(raw) {
