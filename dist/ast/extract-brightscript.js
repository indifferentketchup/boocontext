 ...
 */
export function extractBrightScriptFunctions(content) {
    const exports = [];
 ...
 */
export function extractBrightScriptObservers(content) {
    const out = [];
    const lines = content.split("\n");
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((match = pattern.exec(lines[i])) !== null) {
            ...
            let scope = "other";
            if (/^m\.top$/i.test(receiver))
                scope = "top";
            else if (/^m\.global$/i.test(receiver))
                scope = "global";
 ...
 */
export function extractBrightScriptNavigationCalls(content, helperNames) {
    const out = [];
    if (helperNames.length === 0)
        return out;
    const lines = content.split("\n");
    const alternation = helperNames.map((h) => h.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join("|");
    const pattern = new RegExp(`\\b(?:${alternation})\\s*\\(\\s*([^),]+?)\\s*(?:,\\s*(true|false|\\w+))?\\s*\\)`, "gi");
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        pattern.lastIndex = 0;
        let m;
        while ((m = pattern.exec(line)) !== null) {
            ...
            if (!target || /^["']/.test(target) || /^\d/.test(target))
                continue;
 ...
 */
export function extractBrightScriptShowScreenCalls(content) {
    const out = [];
    const lines = content.split("\n");
 ...
 */
export function extractBrightScriptGraphqlCalls(content) {
    const out = [];
    const lines = content.split("\n");
 ...
 */
export function extractBrightScriptGlobalFields(content) {
    const out = [];
    const lines = content.split("\n");
 ...
 */
export function extractBrightScriptRudderstackEvents(content) {
    const out = [];
    const lines = content.split("\n");
 ...
 */
export function extractFindNodeBindings(content) {
    const result = {};
    const lines = content.split("\n");
