...
// ─── go_router routes ──────────────────────────────────────────────────────
export function extractFlutterRoutes(filePath, content, tags) {
    const routes = [];
...
export function extractFlutterWidgets(filePath, content) {
    const widgets = [];
...
function extractDartConstructorParams(content, className) {
    const props = [];
    // const WidgetName({ required this.param1, this.param2, Key? key })
    const ctorPattern = new RegExp(`${className}\\s*\\(\\s*\\{([^}]+)\\}`, "s");
...
// ─── Dart public exports ───────────────────────────────────────────────────
export function extractDartExports(content) {
    const exports = [];
    ...
    while ((m = fnPattern.exec(content)) !== null) {
        ...
        if (["if", "else", "for", "while", "switch", "return", "await", "async"].includes(name))
            continue;
