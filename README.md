...
| "What models exist?" | ~5K tokens (BOOCONTEXT.md) | ~400 tokens (`database.md`) |
...
| Project | Manual exploration | boocontext scan | boocontext --wiki (targeted) | **Total reduction** |
|---|---|---|---|---|
...
![Token comparison: Without boocontext (46K-66K tokens) vs With boocontext (3K-5K tokens)](assets/token-comparison.jpg)
...
| Project | Full BOOCONTEXT.md | Wiki index only | Index + 1 article | Wiki articles generated |
|---|---|---|---|---|
...
![How boocontext works: Codebase → AST Parser + Regex Fallback → Context Map → CLAUDE.md, .cursorrules, codex.md, MCP Server](assets/how-it-works.jpg)
...
| What boocontext finds | Tokens saved per item | Why |
|---|---|---|
...
| boocontext concept | Roku equivalent |
|---|---|
...
![MCP Server: Claude Code/Cursor ↔ boocontext MCP Server → 6 specialized tools + session cache](assets/mcp-server.jpg)
...
| | boocontext | File concatenation tools | AST-based tools (e.g. code-review-graph) |
|---|---|---|---|
