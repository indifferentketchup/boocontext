...
async function isAllowedRoot(dir: string): Promise<boolean> {
  if (!allowedRoots) return true;
  const realDir = await realpath(resolve(dir)).catch(() => resolve(dir));
  for (const root of allowedRoots) {
    const realRoot = await realpath(root).catch(() => root);
    if (realDir === realRoot || realDir.startsWith(realRoot + sep)) return true;
  }
  return false;
}
...
const TOOLS: ToolDefinition[] = [
  ...
  },
  {
    ...
    description:
      "Get the wiki index (~200 tokens). Lists all available wiki articles with one-line summaries. Read this at session start for instant project orientation. If wiki not generated, run `npx boocontext --wiki` first.",
    inputSchema: {
      type: "object",
      properties: {
        directory: { type: "string", description: "Directory (defaults to cwd)" },
  ...
  },
  {
    ...
    description:
      "Get the knowledge map for a second-brain or docs folder: decisions made, open questions, recurring themes, people mentioned, and an index of all notes by type (ADR, meeting, retro, spec, etc.). Run `npx boocontext --mode knowledge` first to generate it.",
    inputSchema: {
      type: "object",
      properties: {
        directory: { type: "string", description: "Directory (defaults to cwd)" },
      ...
      try {
        const result = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error(`Tool ${toolName} timed out after ${TOOL_CALL_TIMEOUT_MS}ms`)), TOOL_CALL_TIMEOUT_MS)
          ),
        ]);
