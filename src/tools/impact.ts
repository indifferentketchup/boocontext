...
export function createImpactTool(manager: ChildServerManager) {
  return {
    name: "boocontext_impact",
    description:
      "Impact analysis. Merges tree-sitter-analyzer trace_impact (symbol-level) with boocontext blast_radius (file-level). Use before making changes to understand change propagation.",
    inputSchema: {
      type: "object",
      properties: {
    ...
    async handler(args: any): Promise<VerdictEnvelope> {
      ...
      try {
        ...
        let tsaResult: any = null;
        let scannerResult: any = null;
